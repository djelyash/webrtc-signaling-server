...
class SqsApi {
    ...
    async enqueue(connectionId, connectionType, reqLogger) {
        ...
        try {
            ...
            reqLogger.info("Success, message sent. MessageID:", data.MessageId);
            return true;
        } catch (err) {
    ...
    async dequeue(connectionType, reqLogger) {
        if(!this.queueUrls[connectionType]){
            throw {errorCode: 503, error : "Wrong connection Type" + connectionType};
        }
        const receiveParam = {
        ...
        try {
            ...
            if (data.Messages) {
                ...
                try {
                    ...
                    reqLogger.debug("Message Deleted", data);
                } catch (err) {
                    reqLogger.error("Message not Deleted", data);
                    reqLogger.error(err);
                }
                return data.Messages[0].Body;
            } else {
                reqLogger.info("No messages");
                throw {errorCode: 404, error: "no connections available to serve right now try again later"};
            }

        } catch (err) {
            reqLogger.error("Receive Error", err);
            if(err.errorCode) {
                throw err;
            } else{
